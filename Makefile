...
# We need to implement USB support for -usbdevice keyboard or -usbdevice mouse
run: $(ISO)
	qemu-system-i386 -cdrom $^ -monitor stdio -m $(MEM) -smp $(CORES)
run-debug: $(ISO)
	qemu-system-i386 -cdrom $^ -d int,cpu,guest_errors -no-reboot -no-shutdown -m $(MEM) -smp $(CORES)

fat16.img:
	dd if=/dev/zero of=$@ bs=1M count=16
	mkfs.fat -F 16 -n "GECKOOS" $@
